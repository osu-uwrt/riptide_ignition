...
def generate_launch_description():
    ...
    ld.add_action( 
        GroupAction([
            ...
            Node(package='riptide_ignition2', executable='depth_remap', name='depth_remap', respawn=True, output='screen'),
            Node( 
                package='riptide_ignition2', executable='imu_remap', name='imu_remap', respawn=True, output='screen',
                parameters = [
                    {'vehicle_config': configFile}
                ]
            ),
            # Node(
            #     package='riptide_ignition2', executable='thrust_remap', name='thrust_remap', respawn=True, output='screen',
            #     parameters = [
            #         {'vehicle_config': configFile}
            #     ]
            # )
